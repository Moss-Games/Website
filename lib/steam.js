...
// of remote image URLs to download/upload — see docs/GAMES.md for why
// "cover" comes from the first screenshot rather than a Steam capsule image
// (no capsule size matching the card's 616:353 ratio is available without a
// token, and appdetails doesn't return one either).
export function mapSteamDataToGameFields(data) {
  return {
    ...
    images: {
      ...
      cover: data.screenshots?.[0]?.path_full || null,
